...
diag_log '<infiSTAR.de> VERSION: 13-Dec-2016 20-44-58 infiSTAR.de AHAT (v0245a)';
comment 'Antihack & AdminTools - Christian Lorenzen - www.infiSTAR.de - License: (CC)';
UPDATEEMAIL='tgrondal@gmail.com';
INFISTARVERSION='13-Dec-2016 20-44-58 - v0245a';
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!    tgrondal@gmail.com - 13-Dec-2016 20-44-58 - v0245a - %2 - %3',_t,serverName,productVersion];
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [tgrondal@gmail.com - 13-Dec-2016 20-44-58 - v0245a - %2 - %3]',_t,serverName,productVersion];
};
_test = [0,{}] execFSM 'call.fsm';
_test = [0,{}] execFSM 'call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!    tgrondal@gmail.com - 13-Dec-2016 20-44-58 - v0245a - %2 - %3',_t,serverName,productVersion];
...
FNC_A3_LOG_RAND_VAR = compileFinal "
	'ARMA_LOG' callExtension format['A3_RANDOMVAR_LOG:%1 (v0245a)',_this];
	diag_log ('<infiSTAR.de>RandomVarLog| '+_this+' (v0245a)');
";
...
_fnc_RandomGen =
{
	_fnc_actualGen = {
		...
		_arr = ['P','S','7','N','9','I','c','j','n','e','X','f','m','P','l','D','z','H','G','F','f','j','u','O','J','Y','U','8','u','u'];
...
_FNC_AH19_KICKLOG = call _fnc_RandomGen;format['_FNC_AH19_KICKLOG: %1',_FNC_AH19_KICKLOG] call FNC_A3_LOG_RAND_VAR;
_FNC_AH19_KICKLOGSPAWN = call _fnc_RandomGen;format['_FNC_AH19_KICKLOGSPAWN: %1',_FNC_AH19_KICKLOGSPAWN] call FNC_A3_LOG_RAND_VAR;
...
_AH19KICKLOG = call _fnc_RandomGen;format['_AH19KICKLOG: %1',_AH19KICKLOG] call FNC_A3_LOG_RAND_VAR;
...
_AH19_KICKLOG = call _fnc_RandomGen;format['_AH19_KICKLOG: %1',_AH19_KICKLOG] call FNC_A3_LOG_RAND_VAR;
...
FNC_A3_HACKLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_HACKLOG:%1 [FPS: %2|THREADS: %3] (v0245a)',_this,diag_fps,count diag_activeSQFScripts];
	diag_log ('<infiSTAR.de>HL| '+_this+' (v0245a)');
";
FNC_A3_SURVEILLANCELOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_SURVEILLANCELOG:%1 [FPS: %2|THREADS: %3] (v0245a)',_this,diag_fps,count diag_activeSQFScripts];
	diag_log ('<infiSTAR.de>SL| '+_this+' (v0245a)');
";
FNC_A3_ADMINLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_ADMINLOG:%1 (v0245a)',_this];
	diag_log ('<infiSTAR.de>AdminLog| '+_this+' (v0245a)');
";
FNC_A3_CONNECTLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_CONNECTLOG:%1 (v0245a)',_this];
	diag_log ('<infiSTAR.de>ConnectLog| '+_this+' (v0245a)');
";
FNC_A3_TOKENLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_TOKENLOG:%1 (v0245a)',_this];
	diag_log ('<infiSTAR.de>TokenLog| '+_this+' (v0245a)');
";
FNC_A3_RESOLVESTEAMNAMELOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_RESOLVESTEAMNAMELOG:%1 (v0245a)',_this];
	diag_log ('<infiSTAR.de>ResolveSteamName| '+_this+' (v0245a)');
...
_A3AHstring = "
	...
	FNC_INFISERVERKICK = {
		...
		diag_log format['<infiSTAR.de>KickLog| %1(%2)   (v0245a)',_name,_uid];
	};
	...
	FNC_A3_infiSTARBAN = {
		...
		if(_reason == '')then
		{
			'ARMA_LOG' callExtension format['A3_BANLOG:%1(%2) (v0245a)',_name,_uid];
			diag_log format['<infiSTAR.de>BanLog| %1(%2)   (v0245a)',_name,_uid];
		}
		else
		{
			'ARMA_LOG' callExtension format['A3_BANLOG:%1(%2) %3 (v0245a)',_name,_uid,_reason];
			diag_log format['<infiSTAR.de>BanLog| %1(%2) %3 (v0245a)',_name,_uid,_reason];
		};
...
_fnc_infiSTAR_PlayerLog = {
	...
	if({((toLower _name) find _x != -1)} count ['thirtysix','le hippo'] > 0)exitWith{
		...
		[_name,_uid,'SLOG_SKICK',toArray(_log)] call "+_FNC_AH19_KICKLOG+";
	};
...
fnc_infiSTAR_PlayerLog = compileFinal ([_fnc_infiSTAR_PlayerLog] call fnc_CompilableString);
							...
							"; if(_UVC)then{ _A3AHstring = _A3AHstring + "
								if((!("+str _UVW+") && {_vehstype in "+str _ForbiddenVehicles+"}) || (("+str _UVW+") && {!(_vehstype in "+str _VehicleWhiteList+")}))then
								{
									...
									if(_crew isEqualTo [])then
									{
										...
										['SERVER','SERVER','HLOG',toArray(_log)] call "+_FNC_AH19_KICKLOG+";
									...
									else
									{
										...
										_log = format['BadVehicle (S-UVC+LVC): %1',_vehstype];
										{
											_name = name _x;
											_puid = getPlayerUID _x;
											[_name,_puid,'HLOG',toArray(_log)] call "+_FNC_AH19_KICKLOG+";
										} forEach _crew;
										_veh call fnc_deleteObject;
									};
								};
						...
						if(!isNull _veh)then
						{
							if((_plrObj == _veh) && (_vehstype != _vehptype))then
							{
								...
								if!(_veh in _vehicles)then
								{
									...
									[_name,_puid,'SLOG',toArray(_log)] call "+_FNC_AH19_KICKLOG+";
								};
							};
						};
					};
	...
	"+_FNC_AH19_KICKLOG+" = compileFinal ([_FNC_AH19_KICKLOG] call fnc_CompilableString);
	...
	_FNC_PVAH_AdminReq = {
		...
		if(isNil '_clientNetID')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq clientNetID is Nil (v0245a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _clientNetID != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq clientNetID wrong typeName: %1   %2 (v0245a)',typeName _clientNetID,_clientNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(_clientNetID == '')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq clientNetID is empty (v0245a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		_ObjFromNetID = objectFromNetId _clientNetID;
		if(isNil '_ObjFromNetID')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq playerObj is Nil (v0245a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _ObjFromNetID != 'OBJECT')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq ObjFromNetID wrong typeName: %1   %2 (v0245a)',typeName _ObjFromNetID,_ObjFromNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(isNull _ObjFromNetID)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq ObjFromNetID is Null (v0245a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_array')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq is Nil (v0245a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _array != 'ARRAY')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq wrong typeName: %1   %2 (v0245a)',typeName _array,_array];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_option')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq option is Nil (v0245a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _option != 'SCALAR')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq option wrong typeName: %1   %2 (v0245a)',typeName _option,_option];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(typeName _playerObj != 'OBJECT')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq playerObj wrong typeName: %1   %2 (v0245a)',typeName _playerObj,_playerObj];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(isNull _playerObj)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq playerObj is Null (v0245a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if!(_ObjFromNetID isEqualTo _playerObj)then
		{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq playerObj != _ObjFromNetID - %1/2  (v0245a)',_playerObj,_ObjFromNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		...
		if(!isPlayer _playerObj)then
		{
			if(_option == 1234)then
			{
				...
				if(isNil '_puid')exitWith{
					_mytime = call fnc_getserverTime;
					_log = _mytime + 'AdminReq puid is Nil (v0245a)';
					_log call FNC_A3_SURVEILLANCELOG;
				};
				if(typeName _puid != 'STRING')exitWith{
					_mytime = call fnc_getserverTime;
					_log = _mytime + format['AdminReq puid wrong typeName: %1   %2 (v0245a)',typeName _puid,_puid];
					_log call FNC_A3_SURVEILLANCELOG;
				};
				
				_mytime = call fnc_getserverTime;
				_log = format['AdminReq playerObj is not a player   %1(%2 - %3) - trying to resolve player via allPlayers (v0245a)',name _playerObj,getPlayerUID _playerObj,_puid];
				_log = _mytime + _log;
				_log call FNC_A3_SURVEILLANCELOG;
				if(_puid == '')exitWith{};
				{
		...
		if(!isPlayer _playerObj)exitWith{
			_mytime = call fnc_getserverTime;
			_log = format['AdminReq playerObj is not a player   %1(%2) (v0245a)',name _playerObj,getPlayerUID _playerObj];
			_log = _mytime + _log;
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_tokenreceived')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq tokenreceived is Nil (v0245a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _tokenreceived != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq tokenreceived wrong typeName: %1   %2 (v0245a)',typeName _tokenreceived,_tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(_tokenreceived == '')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq tokenreceived is empty (v0245a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(_clientUID isEqualTo '')then
		{
			...
			if(_foundtokenid > 0)then
			{
				...
				if(!isNil '_puidfound')then
				{
					...
					_mytime = call fnc_getserverTime;
					_log = _mytime + format['%1(%2) | AdminReq - SERVER RESOLVED UID! (v0245a)',_clientName,_clientUID];
					_log call FNC_A3_SURVEILLANCELOG;
				};
			};
		};
		...
		if(!_result)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - BadToken %3 (v0245a)',_clientName,_clientUID,_tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		_fnc_NoUID = {
			...
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - !!!!!!ADMIN-LOGIN-ERROR!!!!!! (v0245a)',_clientName,_clientUID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		call {
		...
		{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - Attempted to Use the AdminMenu (v0245a)',_clientName,_clientUID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(_option == 1234)exitWith
		{
			...
			if(_puid == _clientUID)then
			{
				if!(_clientUID in _locDevs)then{
					diag_log format['<infiSTAR.de> ******ADMIN-LOGIN******: %1(%2) (v0245a)',_clientName,_clientUID];
				};
				"; if(_enableIngameLogs)then{ _A3AHstring = _A3AHstring + "
				...
				[
					'',
					{
						...
						"; if(_LogAdminActions)then{ _A3AHstring = _A3AHstring + "
							if(isNil 'fnc_adminLog')then{if(_puid in "+str _devs+")then{fnc_adminLog = compileFinal '';}else{fnc_adminLog = compileFinal '[profileName,getPlayerUID player,''ALOG'',toArray _this] call "+_AH19KICKLOG+";';};};
	...
	_FNC_AH19_KICKLOGSPAWN =
	{
		...
		if(isNil '_input')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'FNC_AH19_KICKLOGSPAWN _this is Nil (v0245a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		diag_log format['<infiSTAR.de FNC_AH19_KICKLOGSPAWN> %1',_this];
		
		_tokenreceived = _this select 0;
		if(isNil '_tokenreceived')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + '_tokenreceived _this is Nil (v0245a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _tokenreceived != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_tokenreceived wrong type %1 (v0245a)',typeName _tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_arraysent')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + '_arraysent _this is Nil (v0245a)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _arraysent != 'ARRAY')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_arraysent wrong type %1 (v0245a)',typeName _arraysent];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(typeName _netId != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_netId wrong type %1 (v0245a)',typeName _netId];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		diag_log format['<infiSTAR.de FNC_AH19_KICKLOGSPAWN> %1(%2) Token %3 - objByNetID %4',_name,_puid,_tokenreceived,_objectFromNetId];
		...
		else
		{
			if(_objectUID isEqualTo _puid)then
			{
				...
				{
					diag_log format['<infiSTAR.de FNC_AH19_KICKLOGSPAWN> %1(%2) PlayerObjectName is not equal to PlayerName!',_name,_puid,_objectName];
					_name = _objectName;
				};
			};
		};
		...
		if(!_result)exitWith
		{
			...
			diag_log format['<infiSTAR.de FNC_AH19_KICKLOGSPAWN> %1',_log];
			[_name,_puid,'SLOG_SKICK',toArray(_log)] call "+_FNC_AH19_KICKLOG+";
		};
		
		
		_arraysent set [0,_objectName];
		_arraysent set [1,_puid];
		_arraysent spawn "+_FNC_AH19_KICKLOG+";
	};
	"+_FNC_AH19_KICKLOGSPAWN+" = compileFinal ([_FNC_AH19_KICKLOGSPAWN] call fnc_CompilableString);
...
_AH_MAIN_BLOCK = {
	...
	if(isNil'"+_AH19KICKLOG+"')exitWith{
	...
	if!(_puid isEqualTo _clientUID)exitWith
	{
		...
		[_name,_puid,'SLOG_SKICK',toArray(_log)] call "+_AH19KICKLOG+";
		(findDisplay 46)closeDisplay 0;
	};
	...
	if!(_puid in _admins)then
	{
		...
		if(_add_Id > -1)then
		{
			...
			if(_announce)then
			{
				...
				[profileName,_puid,'HLOG_SKICK',toArray(_log)] call "+_AH19KICKLOG+";
				[] spawn "+_AHKickOFF+";
			};
		};

		{
			if(!isNil _x)then
			{
				...
				[profileName,_puid,'SLOG_SKICK',toArray(_log)] call "+_AH19KICKLOG+";
				(findDisplay 46)closeDisplay 0;
			};
		} forEach [
		...
		_cfgPatches = 'true' configClasses (configFile >> 'CfgPatches');
		{
			...
			if(_lconfigName in _badclassesban)then
			{
				...
				[_name,_puid,'BAN',toArray(_log)] call "+_AH19KICKLOG+";
				[] spawn "+_AHKickOFF+";
			};
			if(_lconfigName in _badclasseskick)then
			{
				_log = format['Bad Addon found in CfgPatches: %1',_configName];
				[_name,_puid,'HLOG_SKICK',toArray(_log)] call "+_AH19KICKLOG+";
				[] spawn "+_AHKickOFF+";
			};
		} forEach _cfgPatches;
			...
			_fadeOut = {
				...
				if(!isNull findDisplay 602)then
				{
					...
					if(time > _lastCall)then
					{
						...
						[_name,_puid,'HLOG',toArray(_log)] call "+_AH19KICKLOG+";
					};
				};
			};
			_dis_head = 0.20;
...
if(_admin)then
{
	if!(_ltxt isEqualTo '''')then
	{
		if(_ltxt isEqualTo ''!admin'')exitWith
		{
			...
			[_name,_puid,''AC'',toArray(_txt)] call "+_AH19KICKLOG+";
...
"; if(_VDN)then{ _A3AHstring = _A3AHstring + "
	if(_ltxt in [''!vote day'',''/vote day'',''vote day'',''!day'',''/day''])then{
		(findDisplay 24) closeDisplay 0;
		[_name,_puid,''VOTE'',toArray ''DAY''] call "+_AH19KICKLOG+";
	};
	if(_ltxt in [''!vote night'',''/vote night'',''vote night'',''!night'',''/night''])then{
		(findDisplay 24) closeDisplay 0;
		[_name,_puid,''VOTE'',toArray ''NIGHT''] call "+_AH19KICKLOG+";
	};
"; }; _A3AHstring = _A3AHstring + "
false
';
_chatKeyUp_id = -1;
		...
		while{1==1}do
		{
						...
						if(_shape isEqualTo 'POLYLINE')then
						{
							...
							if!(_puid isEqualTo '')then
							{
								...
								if(_puid isEqualTo _varuid)then
								{
									...
									if(_need_hlog)then
									{
										[_name,_varuid,'HLOG',toArray(_log)] call "+_AH19KICKLOG+";
									};
									_need_hlog = false;
								};
							};
			...
			if(_combatTime <= 30)then
			{
				...
				{
					_inCombat = 1;
					[_name,_puid,'COMBATCHECK',_inCombat] call "+_AH19KICKLOG+";
				};
			}
			else
			{
...
_chatKeyUp_id = (finddisplay 24) displayAddEventHandler ['KeyUp',_chatKeyUp];
		...
		_log = 'Loop #0 ended!';
		[profileName,getPlayerUID player,'BAN',toArray(_log)] call "+_AH19KICKLOG+";
		[] spawn "+_AHKickOFF+";
	};
	[_name,_puid,_admins] spawn {
		_name = _this select 0;
									...
									{
										{
											if!((toLower _var) find _x isEqualTo -1)then
											{
												...
												[profileName,getPlayerUID player,'HLOG_SKICK',toArray(_log)] call "+_AH19KICKLOG+";
												[] spawn "+_AHKickOFF+";
												...
												"; if(_UBV2C)then{ _A3AHstring = _A3AHstring + "
													...
													if(!isNil '_m')then
													{
														...
														if((toLower _mstring) find _x != -1)then
														{
															...
															[profileName,getPlayerUID player,'SLOG',toArray(_log)] call "+_AH19KICKLOG+";
															(findDisplay 46)closeDisplay 0;
															missionNamespace setVariable[_var,nil];
														};
													};
		...
		[_name,_puid] spawn {
			...
			_FiredCheck = {
				...
				if(("+str _UFA+") && {((!("+str _UAW+") && {_ammo in "+str _ForbiddenAmmo+"}) || (("+str _UAW+") && {!(_ammo in "+str _AmmoWhiteList+")}))})then
				{
					...
					[profileName,getPlayerUID _unit,'BAN',toArray(_log)] call "+_AH19KICKLOG+";
					[] spawn "+_AHKickOFF+";
				};
								...
								if(_maxAmmo > 1)then
								{
									...
									if(diag_tickTime - "+_lastshotmade+" > 0.2)then
									{
										...
										if((_camm >= _maxAmmo)&&(_muzzle isEqualTo currentWeapon _unit))then
										{
											...
											[profileName,getPlayerUID _unit,'SLOG',toArray(_log)] call "+_AH19KICKLOG+";
				...
				if((!isNull (findDisplay 12))&&(visiblemap))then
				{
					...
					if(!_loggedOnce)then
					{
						...
						if(_size > 5)then
						{
							...
							[_name,_puid,'SLOG',toArray(_log)] call "+_AH19KICKLOG+";
							_loggedOnce = true;
							lbClear _control2;
						};
					};
						...
						if((isNull _driver)||{(player isEqualTo _driver)})then
						{
							if(!isNil '"+_AHpos+"')then
							{
								...
								if(_tmpAHpos isEqualTo [])then
								{
									...
									if(_tpcount > 3)then
									{
										[_name,_puid,'TMPBAN',toArray(_log)] call "+_AH19KICKLOG+";
										[] spawn "+_AHKickOFF+";
									}
									else
									{
								...
								else
								{
									...
									[_name,_puid,'TPLOG',toArray(_log),_tmpAHpos select 0,_tmpAHpos select 1,_lastpos,_curpos] call "+_AH19KICKLOG+";
								};
							}
							else
							{
...
"; }; _A3AHstring = _A3AHstring + "
		...
		[_name,_puid,_admins] spawn {
					...
					[] spawn {
						...
						if(time - "+_clickOnMapTimer+" > 5)then
						{
							...
							if("+_clickOnMapCaught+" >= 3)then
							{
								...
								[profileName,getPlayerUID player,''SLOG_SKICK'',toArray(_log)] call "+_AH19KICKLOG+";
							};
						};
					};
				';
			...
			while{1==1}do
			{
				...
				if(!isNull player)then
				{
					...
					"; if(_LVC)then{ _A3AHstring = _A3AHstring + "
						...
						if(!(player isEqualTo _veh) && !(_vehptype isEqualTo (typeOf _veh)))then
						{
							...
							{
								[_name,_puid,'LVC',player,_vehptype,mapGridPosition _veh] call "+_AH19KICKLOG+";
							};
						};
					"; }; _A3AHstring = _A3AHstring + "
				};
				...
				"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
					...
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Money Hack: [%1] should be [%2]',life_cash,_rnd1])] call "+_AH19KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd2 isEqualTo life_adminlevel)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Adminlevel Hack: [%1] should be [%2]',life_adminlevel,_rnd2])] call "+_AH19KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd3 isEqualTo life_coplevel)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Coplevel Hack: [%1] should be [%2]',life_coplevel,_rnd3])] call "+_AH19KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd4 isEqualTo life_fnc_MPexec)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: life_fnc_MPexec [%1] should be [%2]',life_fnc_MPexec,_rnd4])] call "+_AH19KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd5 isEqualTo life_atmbank)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: life_atmbank [%1] should be [%2]',life_atmbank,_rnd5])] call "+_AH19KICKLOG+";[] spawn "+_AHKickOFF+";
					};
	...
	_hours = floor(serverTime / 60 / 60);_value = ((serverTime / 60 / 60) - _hours);if(_value == 0)then{_value = 0.0001;};_minutes = round(_value * 60);_seconds = '19';
	_devLog = format['<infiSTAR.de> %1 VERSION: 13-Dec-2016 20-44-58 infiSTAR.de AHAT (v0245a) - server running: %2:%3:%4',VERSIONCHECKRESULT,_hours,_minutes,_seconds];diag_log _devLog;
...
_from = 'tgrondal@gmail.com';
_vers = '13-Dec-2016 20-44-58 - v0245a';
...
[] spawn {
			...
			{
				{
					...
					{
						_name = name _x;
						[_name,_puid,'BAN',toArray('')] call "+_FNC_AH19_KICKLOG+";
					};
				} forEach allPlayers;
	...
	"; if((_MOD == 'Epoch')&&(_MPH))then{ _A3AHstring = _A3AHstring + "
		LOG_MPHIT_HANDLER =
		{
			...
			if(_log!='')then
			{
				'ARMA_LOG' callExtension format['A3_MPHIT:%1 (v0245a)',_log];
			};
		};
...
_DO_THIS_MORE_OFTEN = {
	...
	if(hasInterFace)then
	{
		...
		} forEach ['"+_AH19_KICKLOG+"','"+_adminsA+"','"+_MC+"'];
		{
			if(!isNil _x)then
			{
				_var = missionNamespace getVariable _x;
		...
		};
		{
			...
			if(!isNil '_pNs')then
			{
				...
				[profileName,getPlayerUID player,'HLOG',toArray(_log)] call "+_AH19KICKLOG+";
				profileNamespace setVariable[_x,nil];saveprofileNamespace;
			};
		...
		while{1==1}do
		{
			...
			{
				_timer0 = time + 20;
				'"+_AH19_KICKLOG+"' addPublicVariableEventHandler {(_this select 1) call "+_FNC_AH19_KICKLOGSPAWN+";};
				uiSleep 1;
											...
											if!(_uid in "+_tokenSentArray+")then
											{
												if(_uid in "+_connectedArray+")then
												{
													...
													if(_timePassed > 60)then
													{
														...
														[_name,_uid,'HLOG_SKICK',toArray(_log)] call "+_FNC_AH19_KICKLOG+";
													};
												};
											};
										"; }; _A3AHstring = _A3AHstring + "
...
_zeroCode = {
	if(hasInterface)then
	{
		_AHKickOFF = compileFinal '
			sleep 0.1;
			(findDisplay 46)closeDisplay 0;
			if(!isNil''"+_AH19KICKLOG+"'')then{[profileName,getPlayerUID player,''KICKME''] call "+_AH19KICKLOG+";};
		';
		if(isNil'"+_AHKickOFF+"')then{"+_AHKickOFF+" = _AHKickOFF;};
		if(str _AHKickOFF != str "+_AHKickOFF+")then{[] spawn _AHKickOFF;};
		_AH19KICKLOG = compileFinal ""
			if(!isNil'"+_YourPlayerToken+"')then
			{
				"+_AH19_KICKLOG+" = ["+_YourPlayerToken+",_this,netId player];publicVariableServer '"+_AH19_KICKLOG+"';"+_AH19_KICKLOG+"=nil;
			}
			else
			{
				_this spawn {
					...
					if(isNil'"+_YourPlayerToken+"')exitWith{KICKED_FOR_NIL_TOKEN_AH19KICKLOG = format['PLAYER__%1__%2',getPlayerUID player,_this];publicVariableServer 'KICKED_FOR_NIL_TOKEN_AH19KICKLOG';(findDisplay 46)closeDisplay 0;};
					"+_AH19_KICKLOG+" = ["+_YourPlayerToken+",_this,netId player];publicVariableServer '"+_AH19_KICKLOG+"';"+_AH19_KICKLOG+"=nil;
				};
			};
		"";
		if(isNil'"+_AH19KICKLOG+"')then{"+_AH19KICKLOG+" = _AH19KICKLOG;};
		if(str _AH19KICKLOG != str "+_AH19KICKLOG+")then{[] spawn _AHKickOFF;};
...
call compile ("
	FNC_DO_CLIENT_VAR_DUMP = compileFinal ""
		...
		{
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- CURRENTLY DUMPING - WAIT UNTIL IT IS DONE (max 120s) --- (v0245a)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- CURRENTLY DUMPING - WAIT UNTIL IT IS DONE (max 120s) --- (v0245a)',_name,_puid];
		};
		...
		{
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2: Need at least 1 more connected & alive player to dump vars of %1(%2)! (v0245a)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| Need at least 1 more connected & alive player to dump vars of %1(%2)! (v0245a)',_name,_puid];
		};
		...
		{
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- ALREADY DUMPING THIS PLAYER --- (v0245a)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- ALREADY DUMPING THIS PLAYER --- (v0245a)',_name,_puid];
		};
		...
		[_name,_puid] spawn {
			...
			if(time > _timeOut)exitWith
			{
				'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- DUMP TIMED OUT --- (v0245a)',_name,_puid];
				diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- DUMP TIMED OUT --- (v0245a)',_name,_puid];
			};
			...
			{
				'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- ALL GOOD - NOTHING TO DUMP --- (v0245a)',_name,_puid];
				diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- ALL GOOD - NOTHING TO DUMP --- (v0245a)',_name,_puid];
			};
			
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- STARTING DUMP --- (v0245a)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- STARTING DUMP --- (v0245a)',_name,_puid];
			{
			...
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- DUMP ENDED --- (v0245a)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- DUMP ENDED --- (v0245a)',_name,_puid];
		};
	"";
");
if(_testserver)then{
