			...
			if (ctrlText ((findDisplay 63) displayCtrl 101) == localize "str_channel_global") then
			{
				...
				if (disconnect_me == 0) then {
					if (isNil "warn_one") then {
						warn_one = true;
						systemChat ("Please do not use voice on global, this is your first warning.");
						[] spawn DS_slap_them;
						playSound "flashbang";
						["NO VOICE ON GLOBAL"] spawn DS_double_cut;
					};
				};
				if (disconnect_me == 1) then {
					if (isNil "warn_two") then {
						warn_two = true;
						systemChat ("Please do not use voice on global, this is your second warning..");
						[] spawn DS_slap_them;
						playSound "flashbang";
						["NO VOICE ON GLOBAL"] spawn DS_double_cut;
					};
				...
				if (disconnect_me == 2) then {
					if (isNil "warn_three") then {
						warn_three = true;
						systemChat ("PLEASE DO NOT USE VOICE ON GLOBAL! This is your LAST warning!");
						systemChat ("You will be frozen and then disconnected!");
						[] spawn DS_slap_them;
						playSound "flashbang";
						["NO VOICE ON GLOBAL! This is your LAST warning!"] spawn DS_double_cut;
					};
				};
