...
if (_curTarget isKindOf "House_F" && playerSide isEqualTo west) exitWith {
    if ((nearestObject [[15811.9,16584.5,0],"Land_MilOffices_V1_F"]) == _curTarget || (nearestObject [[8614.38,10037.2,0],"Land_Research_house_V1_F"]) == _curTarget) then {
...
if (!(_curTarget in life_vehicles) || isNil {_curTarget getVariable "house_owner"}) then {
    ...
    if ((typeOf _curTarget) in ["Land_i_Garage_V1_F","Land_i_Garage_V2_F","Land_GarageShelter_01_F","Land_House_Big_03_F"]) then {
