...
if (isNull _unit || isNull _shooter) exitWith {player allowDamage true; life_isrubbered = false;};

if (_shooter isKindOf "Man" && alive player) then {
    if (!life_isrubbered) then {
        life_isrubbered = true;
        ...
        [0,"STR_NOTF_DownRB",true,[profileName, _shooter getVariable ["realname",name _shooter]]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
